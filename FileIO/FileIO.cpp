...
namespace Uplinkzero
{
    ...
    FileIO::FileIO(const char *path, bool write /*= false*/, bool read /* = true*/)
        : m_handle(nullptr), m_fileSize(0), m_cursor(0), m_error(false), m_openRead(false), m_openWrite(false), m_filePath(path)
    {
        if (write && read)
        {
    ...
    size_t FileIO::ReadBlock(size_t size, BYTE *pBuffer)
    {
        if (m_error == true)
            return 0;
    ...
    size_t FileIO::WriteBlock(size_t size, BYTE *pBuffer)
    {
        if (m_error == true)
            return 0;
